/**
 * Determine if Easy Digital Downloads settings are enabled for the block.
 *
 * @since 1.0.0
 ...
 * @return {boolean}		        Does the block have Easy Digital Downloads settings
 */
export default function hasEDD(
	controls,
	hasControlSets,
	enabledControls,
	variables
	...
	if (
		! pluginActive ||
		! enabledControls.some( ( control ) => control.settingSlug === 'edd' )
	) {
		return false;
	}

	if ( hasControlSets && ! controls.hasOwnProperty( 'edd' ) ) {
		return false;
	}

	// Could add more robust logic in the future, but for now, show the
	// indicator is there are any WooCommerce settings.
	const ruleSets = controls?.edd?.ruleSets ?? [];
